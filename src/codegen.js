...
const whileGen = node => {
  ...
  if (node.con.type === "B_EXP") {
    const leftCon = node.con.left
    const rightCon = node.con.right
    let operator = node.con.operator
    
    if (operator === "!=") operator += "="
    if (operator === "==") operator += "="

    const whileBody = genrateCode({ body: node.body })
      .split('\n')
      .map(ln => ln.trim() !== "" ? `  ${ln}` : '')
      .join("\n")
